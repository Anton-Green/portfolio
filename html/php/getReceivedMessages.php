...
$sql = "SELECT message_text FROM chat WHERE receiver_id = ? AND sender_id = (SELECT id FROM users WHERE username = ?)";
...
echo json_encode(['status' => 'success', 'messages' => $receivedMessages]);
